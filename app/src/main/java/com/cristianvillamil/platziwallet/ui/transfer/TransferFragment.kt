...
class TransferFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        transferButton.setOnClickListener {
            ApplicationDatabase.getAppDatabase(context!!)?.getDAO()?.saveTransfer(
                TransferEntity(
                    userId = "12313",
                    username = "abcdestebano",
                    transactionDate = "12/12/20",
                    transactionAmount = "50.000",
                    receiverUserId = "34123"
                )
            )
            val runnable = Runnable {
                var userTransferString = ""
                val transferList = ApplicationDatabase.getAppDatabase(context!!)?.getDAO()
                    ?.findTransferByUserName("abcdestebano")
